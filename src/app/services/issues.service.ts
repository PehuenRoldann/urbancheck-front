...
export class IssuesService {
  ...
  async fetchIssues(): Promise<void> {
    ...
    const data = await client.request<{ findAllIssues: Issue[] }>(IssueQueries.findAllIssues);
    console.log("Data fetched from GraphQL:", data); // DEBUG borrar antes de prod
    this.issuesListSubject.next(data.findAllIssues ?? []);
    console.log('Fetched issues:', data.findAllIssues); // DEBUG borrar antes de prod
  }
